...
function LessonDetail({ lesson, flashcardsData, questionsData, categories, onBack, onQuiz, speakingId, onSpeak, speechSupported, isFirstLesson, isLastLesson, onPrevLesson, onNextLesson }) {
  ...
  // so it never duplicates a real screenshot already shown above.
  const categoryScreenshotKey = !walkthrough && !MockupComp && lessonCatKeys.length === 1
    ? (categories || []).find((c) => c.key === lessonCatKeys[0])?.screenshot
    : null;
