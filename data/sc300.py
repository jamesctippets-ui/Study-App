...
Beyond an organization's own employees, B2B collaboration lets a partner's users sign in with their own home tenant's credentials as guest users, with their password and MFA status remaining entirely the partner's responsibility -- your tenant only decides what that guest can access, and cross-tenant access settings further control, per partner organization, whether to trust that partner's own MFA and device-compliance claims instead of re-enforcing your own. Day-to-day account administration has its own set of tools: bulk user operations use a Microsoft-provided CSV template to create, update, or delete many accounts in one pass rather than one at a time, and a deleted user is only soft-deleted, recoverable along with its group memberships and licenses for 30 days before being purged permanently. Custom security attributes extend how users can be targeted beyond built-in directory fields -- an administrator-defined key-value pair like Project=Falcon can drive both dynamic group membership rules and attribute-based access conditions, though managing them requires its own dedicated Attribute Assignment/Definition Administrator role, one even a Global Administrator does not hold by default.""",
